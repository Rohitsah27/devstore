...
<div class="max-w-4xl mx-auto bg-gray-800 rounded-lg shadow-lg overflow-hidden border border-gray-700">
    ...
    <form action="{{ route('notes.update', $note['id']) }}" method="POST" class="p-6 space-y-6">
        @csrf
        @method('PUT')
        
        <div>
            <label for="title" class="block text-sm font-medium text-gray-300 mb-1">Title</label>
            <input type="text" name="title" id="title" value="{{ $note['title'] ?? '' }}" required class="w-full bg-gray-700 border border-gray-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors">
        </div>

        <div>
            <label for="content" class="block text-sm font-medium text-gray-300 mb-1">Content (Markdown)</label>
            <textarea name="content" id="content" class="hidden">{{ $note['content'] ?? '' }}</textarea>
        </div>

        <div>
            <label for="tags" class="block text-sm font-medium text-gray-300 mb-1">Tags</label>
            <input type="text" name="tags" id="tags" value="{{ isset($note['tags']) ? implode(', ', $note['tags']) : '' }}" class="w-full bg-gray-700 border border-gray-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors">
            <p class="mt-1 text-xs text-gray-500">Separate tags with commas.</p>
        </div>

        <div class="flex items-center">
            <div class="flex items-center h-5">
                <input id="is_pinned" name="is_pinned" type="checkbox" value="1" {{ ($note['is_pinned'] ?? false) ? 'checked' : '' }} class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-600 rounded bg-gray-700">
            </div>
